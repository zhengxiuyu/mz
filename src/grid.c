...
int mz_update_grid(mz_grid *grid, const mz_fluid *fluid) {
    size_t size;
    unsigned int i, a, b;
    int index;
    ...
    /* count # of fluid for each cell */
    for (i = 0; i < fluid->num_particles; i++) {
        if (!mz_grid_index_from_position(grid, &index, fluid->positions[i])) {
    ...
    memset(grid->num_particles, 0, size);
    for (i = 0; i < fluid->num_particles; i++) {
        mz_grid_index_from_position(grid, &index, fluid->positions[i]);
